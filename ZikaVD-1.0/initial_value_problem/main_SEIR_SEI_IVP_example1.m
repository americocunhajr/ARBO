...
% -----------------------------------------------------------------
% Initial value problem: main_SEIR_SEI_IVP_example1.m
%
...
% transmission rates
betaH = 1/10.4; % vector-to-human (days^-1) 
...
hold off
    ...
    legend(fig1,leg,'FontSize',15,'location','northwest');
    
    % axis limits
    xlim([t0 t1]);
    ylim([0 3.2e5]);

    saveas(figure(1),'fig_IVP_example1_human_compart.png')
...
hold off
    ...
    saveas(figure(2),'fig_IVP_example1_vector_compart.png')
...
fig3 = scatter([1:length(NC)],NC);
    ...
    saveas(figure(3),'fig_IVP_example1_NC.png')
% -----------------------------------------------------------------
